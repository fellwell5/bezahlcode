<?php
	...
	*/
	class Bezahlcode {
		 ...
		 *
		 * @param string $qrmethod optional Will be used in a newer version.
		 * 		Defaults to 'url'.
		 *
		 * @access public
		 * @since Method available since Release 0.1
		 */
		function __construct($iban, $bic, $name, $qrmethod = "url"){
			$this->iban = $iban;
			$this->bic = $bic;
			$this->name = $name;
		...
		private function selectType($type=''){
			switch(strtolower($type)){
				case "gif": $ctype="image/gif"; $ext = "gif"; break;
				case "png": $ctype="image/png"; $ext = "png"; break;
				default: $ctype="image/jpeg"; $ext = "jpg";
			}
			return array($ctype, $ext);
		}
		...
		public function generateBase64($type=''){
			...
			$img = str_replace('data:'.$type[0].';base64,', '', $this->base64);
			$img = str_replace(' ', '+', $img);
